...
class YouTubeUtils {
  ...
  static async hasTranscript() {
    try {
      // Try to find transcript button without opening it
      const expandButton = document.querySelector('tp-yt-paper-button#expand');
      if (expandButton && expandButton.textContent.includes('more')) {
        expandButton.click();
        await this.wait(300);
      ...
      const transcriptButton = Array.from(document.querySelectorAll('button, yt-button-renderer, tp-yt-paper-button'))
        .find(btn => btn.textContent.toLowerCase().includes('transcript'));

      return transcriptButton !== undefined;
    } catch (error) {
      return false;
    }
  }
